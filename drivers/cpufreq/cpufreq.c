...
void cpufreq_boot_limit_update(int period)
{
	...
	for_each_online_cpu(cpu)
	{
		...
		if (new_policy.min > new_policy.user_policy.max || new_policy.max < new_policy.user_policy.min)
		{
...
#define store_one(file_name, object)                                                             \
	static ssize_t store_##file_name(struct cpufreq_policy *policy, const char *buf, size_t count) \
	{                                                                                              \
		int ret, temp;                                                                               \
		struct cpufreq_policy new_policy;                                                            \
                                                                                                 \
	...
	for_each_active_policy(policy)
	{
		if (cpufreq_driver->resume && cpufreq_driver->resume(policy))
			pr_err("%s: Failed to resume driver: %p\n", __func__,
						 policy);
		else if (__cpufreq_governor(policy, CPUFREQ_GOV_START) || __cpufreq_governor(policy, CPUFREQ_GOV_LIMITS))
															...
															unsigned int event)
{
	...
	*/
	...
	if ((policy->governor_enabled && event == CPUFREQ_GOV_START) || (!policy->governor_enabled && (event == CPUFREQ_GOV_LIMITS || event == CPUFREQ_GOV_STOP)))
	{
