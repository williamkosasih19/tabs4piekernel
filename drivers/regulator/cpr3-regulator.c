...
{
	if (ctrl->cpr_enabled && !(ctrl->aggr_corner.sdelta && ctrl->aggr_corner.sdelta->allow_boost))
...
static inline int cpr3_ctrl_clear_cpr4_config(struct cpr3_controller *ctrl)
{
	...
	if (!aggr_sdelta || !(aggr_sdelta->allow_core_count_adj || aggr_sdelta->allow_temp_adj || aggr_sdelta->allow_boost))
	...
	cpr3_masked_write(ctrl, CPR4_REG_MARGIN_ADJ_CTL,
										CPR4_MARGIN_ADJ_CTL_MAX_NUM_CORES_MASK | CPR4_MARGIN_ADJ_CTL_CORE_ADJ_EN | CPR4_MARGIN_ADJ_CTL_TEMP_ADJ_EN | CPR4_MARGIN_ADJ_CTL_KV_MARGIN_ADJ_EN | CPR4_MARGIN_ADJ_CTL_BOOST_EN | CPR4_MARGIN_ADJ_CTL_HW_CLOSED_LOOP_EN_MASK, 0);
...
static int cpr3_regulator_init_thread(struct cpr3_thread *thread)
{
	...
	reg = (thread->consecutive_up << CPR3_THRESH_CONS_UP_SHIFT) & CPR3_THRESH_CONS_UP_MASK;
	reg |= (thread->consecutive_down << CPR3_THRESH_CONS_DOWN_SHIFT) & CPR3_THRESH_CONS_DOWN_MASK;
	reg |= (thread->up_threshold << CPR3_THRESH_UP_THRESH_SHIFT) & CPR3_THRESH_UP_THRESH_MASK;
	reg |= (thread->down_threshold << CPR3_THRESH_DOWN_THRESH_SHIFT) & CPR3_THRESH_DOWN_THRESH_MASK;
...
static int cpr3_regulator_init_cpr4(struct cpr3_controller *ctrl)
{
	...
	if (!ctrl->allow_core_count_adj && !ctrl->allow_temp_adj && !ctrl->allow_boost)
	{
		/*
	...
	if (ctrl->voltage_settling_time)
	{
		 ...
		 * one VDD supply step.
		 */
		temp = (u64)ctrl->cpr_clock_rate * (u64)ctrl->voltage_settling_time;
	...
	for (i = 0; i < ctrl->thread_count; i++)
	{
		...
		for (j = 0; j < thread->vreg_count; j++)
		{
			...
			thread_max_core_count = max(thread_max_core_count,
																	vreg->max_core_count);
			thread_valid_sdelta |= (vreg->allow_core_count_adj | vreg->allow_temp_adj | vreg->allow_boost);
		}
		if (thread_valid_sdelta)
		{
			...
			sdelta->table = devm_kcalloc(ctrl->dev,
																	 thread_max_core_count * ctrl->temp_band_count,
																	 sizeof(*sdelta->table),
																	 GFP_KERNEL);
			if (!sdelta->table)
				return -ENOMEM;
	...
	if (ctrl_valid_sdelta)
	{
		...
		sdelta->table = devm_kcalloc(ctrl->dev, ctrl_max_core_count * ctrl->temp_band_count,
																				...
																				int addr, int *temp_core_adj)
{
	...
	for (i = 0; i < ctrl->temp_band_count; i++)
	{
		...
		reg |= (margin_steps & CPR4_MARGIN_TEMP_CORE_ADJ_MASK) << (i * CPR4_MARGIN_TEMP_CORE_ADJ_SHIFT);
	}
...
static int cpr3_controller_program_sdelta(struct cpr3_controller *ctrl)
{
	...
	if (!sdelta->allow_core_count_adj && !sdelta->allow_temp_adj && !sdelta->allow_boost)
	{
		/*
	...
	cpr3_masked_write(ctrl, CPR4_REG_MARGIN_ADJ_CTL,
										CPR4_MARGIN_ADJ_CTL_MAX_NUM_CORES_MASK | CPR4_MARGIN_ADJ_CTL_CORE_ADJ_EN | CPR4_MARGIN_ADJ_CTL_TEMP_ADJ_EN | CPR4_MARGIN_ADJ_CTL_KV_MARGIN_ADJ_EN | CPR4_MARGIN_ADJ_CTL_BOOST_EN,
										max_core_count << CPR4_MARGIN_ADJ_CTL_MAX_NUM_CORES_SHIFT | ((sdelta->allow_core_count_adj || sdelta->allow_boost) ? CPR4_MARGIN_ADJ_CTL_CORE_ADJ_EN : 0) | ((sdelta->allow_temp_adj && ctrl->supports_hw_closed_loop && sdelta->allow_core_count_adj) ? CPR4_MARGIN_ADJ_CTL_TEMP_ADJ_EN : 0) | (((ctrl->use_hw_closed_loop && !sdelta->allow_boost) || !ctrl->supports_hw_closed_loop) ? CPR4_MARGIN_ADJ_CTL_KV_MARGIN_ADJ_EN : 0) | (sdelta->allow_boost ? CPR4_MARGIN_ADJ_CTL_BOOST_EN : 0));
...
static int cpr3_regulator_init_cprh_corners(struct cpr3_regulator *vreg)
{
	...
	for (i = 0; i < vreg->corner_count; i++)
	{
		...
		floor_volt_steps = DIV_ROUND_UP(corner->floor_volt -
																				ctrl->base_volt,
																		ctrl->step_volt);
		delta_quot_steps = corner->proc_freq ? DIV_ROUND_UP(corner->target_quot[ro_sel] -
																														base_quots[ro_sel],
		...
		reg = (open_loop_volt_steps << CPRH_CORNER_INIT_VOLTAGE_SHIFT) & CPRH_CORNER_INIT_VOLTAGE_MASK;
		reg |= (floor_volt_steps << CPRH_CORNER_FLOOR_VOLTAGE_SHIFT) & CPRH_CORNER_FLOOR_VOLTAGE_MASK;
		reg |= (delta_quot_steps << CPRH_CORNER_QUOT_DELTA_SHIFT) & CPRH_CORNER_QUOT_DELTA_MASK;
		reg |= (ro_sel << CPRH_CORNER_RO_SEL_SHIFT) & CPRH_CORNER_RO_SEL_MASK;
		...
		struct cpr3_controller *ctrl)
{
	...
	{
		reg |= (i < vreg->corner_band_count ? vreg->corner_band[i].corner & CPRH_CORNER_BAND_MASK : vreg->corner_count + 1)
	...
	cpr3_masked_write(ctrl, CPR4_REG_MARGIN_ADJ_CTL,
										CPR4_MARGIN_ADJ_CTL_MAX_NUM_CORES_MASK | CPR4_MARGIN_ADJ_CTL_CORE_ADJ_EN | CPR4_MARGIN_ADJ_CTL_TEMP_ADJ_EN | CPR4_MARGIN_ADJ_CTL_KV_MARGIN_ADJ_EN | CPR4_MARGIN_ADJ_CTL_HW_CLOSED_LOOP_EN_MASK,
										vreg->max_core_count << CPR4_MARGIN_ADJ_CTL_MAX_NUM_CORES_SHIFT | ((vreg->allow_core_count_adj) ? CPR4_MARGIN_ADJ_CTL_CORE_ADJ_EN : 0) | (vreg->allow_temp_adj ? CPR4_MARGIN_ADJ_CTL_TEMP_ADJ_EN : 0) | ((ctrl->use_hw_closed_loop) ? CPR4_MARGIN_ADJ_CTL_KV_MARGIN_ADJ_EN : 0) | (ctrl->use_hw_closed_loop ? CPR4_MARGIN_ADJ_CTL_HW_CLOSED_LOOP_ENABLE : 0));
...
static int cpr3_regulator_init_cprh(struct cpr3_controller *ctrl)
{
	...
	if (ctrl->corner_switch_delay_time)
	{
		 ...
		 */
		temp = (u64)ctrl->cpr_clock_rate * (u64)ctrl->corner_switch_delay_time;
		do_div(temp, 1000000000);
	...
	cpr3_masked_write(ctrl, CPRH_REG_CTL, CPRH_CTL_BASE_VOLTAGE_MASK | CPRH_CTL_VOLTAGE_MULTIPLIER_MASK | CPRH_CTL_OSM_ENABLED, reg);
...
static int cpr3_regulator_init_ctrl(struct cpr3_controller *ctrl)
{
	...
	do_div(temp, 1000000000);
	cont_dly = temp;
	if (ctrl->supports_hw_closed_loop && ctrl->ctrl_type == CPR_CTRL_TYPE_CPR3)
	...
	val = (ctrl->idle_clocks << CPR3_CPR_CTL_IDLE_CLOCKS_SHIFT) & CPR3_CPR_CTL_IDLE_CLOCKS_MASK;
	val |= (ctrl->count_mode << CPR3_CPR_CTL_COUNT_MODE_SHIFT) & CPR3_CPR_CTL_COUNT_MODE_MASK;
	val |= (ctrl->count_repeat << CPR3_CPR_CTL_COUNT_REPEAT_SHIFT) & CPR3_CPR_CTL_COUNT_REPEAT_MASK;
	...
	val = (ctrl->step_quot_init_min << CPR3_CPR_STEP_QUOT_MIN_SHIFT) & CPR3_CPR_STEP_QUOT_MIN_MASK;
	val |= (ctrl->step_quot_init_max << CPR3_CPR_STEP_QUOT_MAX_SHIFT) & CPR3_CPR_STEP_QUOT_MAX_MASK;
...
static void cpr3_regulator_set_target_quot(struct cpr3_thread *thread)
{
	...
	if (thread->aggr_corner.ro_mask == CPR3_RO_MASK && thread->last_closed_loop_aggr_corner.ro_mask == CPR3_RO_MASK)
	{
	...
	else if (thread->aggr_corner.ro_mask != thread->last_closed_loop_aggr_corner.ro_mask)
	{
		cpr3_write(thread->ctrl, CPR3_REG_RO_MASK(thread->thread_id),
																							...
																							int vdd_volt, u32 reg_last_measurement)
{
	...
	if (vreg->thread->last_closed_loop_aggr_corner.ro_mask == CPR3_RO_MASK || !vreg->aggregated)
	{
		return;
	}
	...
	else if (ctrl->thread_count == 1 && vdd_volt >= corner->floor_volt && vdd_volt <= corner->ceiling_volt)
	{
	...
	step_dn = !!(reg_last_measurement & CPR3_LAST_MEASUREMENT_THREAD_DN(id));
	step_up = !!(reg_last_measurement & CPR3_LAST_MEASUREMENT_THREAD_UP(id));
	aggr_step_dn = !!(reg_last_measurement & CPR3_LAST_MEASUREMENT_AGGR_DN);
	aggr_step_mid = !!(reg_last_measurement & CPR3_LAST_MEASUREMENT_AGGR_MID);
	...
	pd_valid = !((((reg_last_measurement & CPR3_LAST_MEASUREMENT_PD_BYPASS_MASK) >> CPR3_LAST_MEASUREMENT_PD_BYPASS_SHIFT) & vreg->pd_bypass_mask) == vreg->pd_bypass_mask);
	...
	else if (aggr_step_dn && step_dn && vdd_volt < corner->last_volt && vdd_volt >= corner->floor_volt)
	{
		corner->last_volt = vdd_volt;
	}
	else if (aggr_step_up && step_up && vdd_volt > corner->last_volt && vdd_volt <= corner->ceiling_volt)
	{
		corner->last_volt = vdd_volt;
	}
	else if (aggr_step_mid && vdd_volt >= corner->floor_volt && vdd_volt <= corner->ceiling_volt)
	{
		corner->last_volt = vdd_volt;
	}
	else if (saw_error && (vdd_volt == corner->ceiling_volt || vdd_volt == corner->floor_volt))
	{
		corner->last_volt = vdd_volt;
	}
	else
																									...
																									int new_volt)
{
	...
	if (((last_volt < mem_acc_volt && mem_acc_volt <= new_volt) || (last_volt >= mem_acc_volt && mem_acc_volt > new_volt)))
	{
		...
		mem_acc_corn = new_volt < mem_acc_volt ? ctrl->mem_acc_corner_map[CPR3_MEM_ACC_LOW_CORNER] : ctrl->mem_acc_corner_map[CPR3_MEM_ACC_HIGH_CORNER];
																					 ...
																					 int vdd_volt, int vdd_ceiling_volt)
{
	...
	rc = regulator_set_voltage(ldo_reg, bhs_volt, min(vdd_ceiling_volt, vreg->ldo_max_volt));
	if (rc)
	{
																					...
																					struct cpr3_corner *aggr_corner)
{
	...
	for (i = 0; i < ctrl->thread_count; i++)
	{
		for (j = 0; j < ctrl->thread[i].vreg_count; j++)
		{
			...
			if (!vreg->vreg_enabled || vreg->current_corner == CPR3_REGULATOR_CORNER_INVALID)
				continue;
			...
			ldo_volt = current_corner->open_loop_volt - vreg->ldo_adjust_volt;
			ref_volt = ctrl->use_hw_closed_loop ? aggr_corner->floor_volt : new_volt;

			if (ref_volt < ldo_volt + vreg->ldo_min_headroom_volt || ldo_volt < ctrl->system_supply_max_volt - vreg->ldo_max_headroom_volt ||
			...
			safe_volt = min(max(ldo_volt,
													ctrl->system_supply_max_volt - vreg->ldo_max_headroom_volt),
											vreg->ldo_max_volt);
																					...
																					int new_volt, int last_volt)
{
	...
	ref_volt = ctrl->use_hw_closed_loop ? vdd_floor_volt : new_volt;
																						 ...
																						 struct cpr3_corner *aggr_corner)
{
			 ...
			 (*last_volt >= mem_acc_volt && mem_acc_volt > ref_volt)))
	{
		...
		rc = regulator_set_voltage(ctrl->vdd_regulator, safe_volt,
															 new_volt < *last_volt ? ctrl->aggr_corner.ceiling_volt : new_volt);
		if (rc)
		{
		...
		*last_volt = safe_volt;
		...
		mem_acc_corn = ref_volt < mem_acc_volt ? ctrl->mem_acc_corner_map[CPR3_MEM_ACC_LOW_CORNER] : ctrl->mem_acc_corner_map[CPR3_MEM_ACC_HIGH_CORNER];
																									 ...
																									 struct cpr3_corner *aggr_corner)
{
	...
	if (ctrl->apm && apm_volt > 0 && ((*last_volt < apm_volt && apm_volt <= new_volt) || (*last_volt >= apm_volt && apm_volt > new_volt)))
		apm_crossing = true;
																						...
																						struct cpr3_corner *aggr_corner)
{
	...
	if (new_volt == last_volt && ctrl->supports_hw_closed_loop && ctrl->ctrl_type == CPR_CTRL_TYPE_CPR4)
	{
		/*
																								 ...
																								 u32 reg_last_measurement)
{
	...
	{
		valid = !!(reg_last_measurement & CPR3_LAST_MEASUREMENT_VALID);
		bypass_bits = (reg_last_measurement & CPR3_LAST_MEASUREMENT_PD_BYPASS_MASK) >> CPR3_LAST_MEASUREMENT_PD_BYPASS_SHIFT;
	}
	else
	{
		/*
	...
	for (i = 0; i < ctrl->thread_count; i++)
	{
		for (j = 0; j < ctrl->thread[i].vreg_count; j++)
		{
			...
			pd_valid = !((bypass_bits & vreg->pd_bypass_mask) == vreg->pd_bypass_mask);

			if (!valid || !pd_valid)
				dynamic_floor_volt = max(dynamic_floor_volt,
																 vreg->corner[vreg->dynamic_floor_corner].last_volt);
		}
	}
		...
		const struct cpr3_corner *corner, int step_volt)
{
	...
	if (aggr_corner->open_loop_volt < corner->open_loop_volt)
	{
		...
		sdelta_size = aggr_sdelta->max_core_count * aggr_sdelta->temp_band_count;

		if (aggr_sdelta->allow_core_count_adj || aggr_sdelta->allow_temp_adj)
			memset(aggr_sdelta->table, 0, sdelta_size * sizeof(*aggr_sdelta->table));
		...
		if (sdelta->allow_boost)
		{
			memcpy(aggr_sdelta->boost_table, sdelta->boost_table,
						 sdelta->temp_band_count * sizeof(*sdelta->boost_table));
		...
		else if (aggr_sdelta->allow_boost)
		{
			...
			{
				adjust_sdelta = (corner->open_loop_volt - aggr_corner->open_loop_volt) / step_volt;
				aggr_sdelta->boost_table[i] += adjust_sdelta;
				aggr_sdelta->boost_table[i] = min(aggr_sdelta->boost_table[i], 0);
			}
		}

		aggr_corner->open_loop_volt = corner->open_loop_volt;
		aggr_sdelta->allow_temp_adj = sdelta->allow_temp_adj;
		aggr_sdelta->allow_core_count_adj = sdelta->allow_core_count_adj;
	...
	else if (aggr_corner->open_loop_volt > corner->open_loop_volt)
	{
		...
		if (sdelta->allow_boost)
		{
			for (i = 0; i < aggr_sdelta->temp_band_count; i++)
			{
				adjust_sdelta = (aggr_corner->open_loop_volt - corner->open_loop_volt) / step_volt;
	...
	else
	{
		...
		if (aggr_sdelta->allow_temp_adj && sdelta->allow_temp_adj)
		{
			...
			{
				aggr_index = aggr_core_count * temp_band_count + j;
																						 ...
																						 int step_volt)
{
	...
	aggr_corner->ceiling_volt = max(aggr_corner->ceiling_volt, corner->ceiling_volt);
	aggr_corner->floor_volt = max(aggr_corner->floor_volt, corner->floor_volt);
	aggr_corner->last_volt = max(aggr_corner->last_volt, corner->last_volt);
	aggr_corner->system_volt = max(aggr_corner->system_volt, corner->system_volt);
	aggr_corner->mem_acc_volt = max(aggr_corner->mem_acc_volt, corner->mem_acc_volt);
	...
	if (aggr_quot)
	{
		...
		for (i = 0; i < CPR3_RO_COUNT; i++)
			aggr_corner->target_quot[i] = max(aggr_corner->target_quot[i],
																				corner->target_quot[i]);
	}

	if (aggr_corner->sdelta && corner->sdelta && (aggr_corner->sdelta->table || aggr_corner->sdelta->boost_table))
	{
...
static int _cpr3_regulator_update_ctrl_state(struct cpr3_controller *ctrl)
{
	...
	if (ctrl->cpr_enabled && ctrl->use_hw_closed_loop &&
		...
		reg_last_measurement = cpr3_read(ctrl, CPR3_REG_LAST_MEASUREMENT);
	...
	if (aggr_corner.sdelta)
	{
		...
		if (sdelta_table)
		{
			...
			memset(sdelta_table, 0, sdelta_size * sizeof(*sdelta_table));
		}

		boost_table = sdelta->boost_table;
		if (boost_table)
			memset(boost_table, 0, sdelta->temp_band_count * sizeof(*boost_table));
	 ...
	 */
	if (ctrl->use_hw_closed_loop && aggr_corner.ceiling_volt >= ctrl->apm_threshold_volt && aggr_corner.floor_volt < ctrl->apm_threshold_volt)
	{

		if (aggr_corner.open_loop_volt >= ctrl->apm_threshold_volt - ctrl->apm_adj_volt)
			aggr_corner.floor_volt = ctrl->apm_threshold_volt;
		else
			aggr_corner.ceiling_volt = ctrl->apm_threshold_volt - ctrl->step_volt;

		aggr_corner.last_volt = max(aggr_corner.last_volt, aggr_corner.floor_volt);
		aggr_corner.last_volt = min(aggr_corner.last_volt, aggr_corner.ceiling_volt);
		aggr_corner.open_loop_volt = max(aggr_corner.open_loop_volt, aggr_corner.floor_volt);
		aggr_corner.open_loop_volt = min(aggr_corner.open_loop_volt, aggr_corner.ceiling_volt);
	}

	if (ctrl->use_hw_closed_loop && aggr_corner.ceiling_volt >= ctrl->mem_acc_threshold_volt && aggr_corner.floor_volt < ctrl->mem_acc_threshold_volt)
	{
	...
	if (ctrl->cpr_enabled && ctrl->last_corner_was_closed_loop)
	{
		...
		else if (ctrl->ctrl_type == CPR_CTRL_TYPE_CPR4 && ctrl->supports_hw_closed_loop)
	...
	if (ctrl->ctrl_type == CPR_CTRL_TYPE_CPR4 && ctrl->supports_hw_closed_loop)
	{
		/*
	...
	if (ctrl->cpr_enabled)
	{
		...
		for (i = 0; i < ctrl->thread_count; i++)
		{
			...
			for (j = 0; j < ctrl->thread[i].vreg_count; j++)
			{
				...
				if (vreg->vreg_enabled)
					vreg->last_closed_loop_corner = vreg->current_corner;
			}
		}

		if (ctrl->proc_clock_throttle)
		{
			if (aggr_corner.ceiling_volt > aggr_corner.floor_volt && (ctrl->use_hw_closed_loop || new_volt < aggr_corner.ceiling_volt))
	...
	else if (ctrl->ctrl_type == CPR_CTRL_TYPE_CPR4 && ctrl->vdd_limit_regulator)
	{
		/* Set ceiling and floor limits in hardware */
		rc = regulator_set_voltage(ctrl->vdd_limit_regulator,
															 aggr_corner.floor_volt,
																				...
																				struct cpr3_aging_sensor_info *aging_sensor)
{
	...
	if (ctrl->aging_gcnt_scaling_factor)
		gcnt = gcnt_ref * ctrl->aging_gcnt_scaling_factor / CPR3_AGING_GCNT_SCALING_UNITY;
	...
	for (i = 0; i <= ctrl->sensor_count / 32; i++)
	{
		...
		if (aging_sensor->sensor_id / 32 >= i && aging_sensor->sensor_id / 32 < (i + 1))
	...
	for (i = 0; i < CPR3_AGING_MEASUREMENT_ITERATIONS; i++)
	{
		...
		reg = cpr3_read(ctrl, CPR3_REG_CPR_STATUS);
		is_aging_measurement = reg & CPR3_CPR_STATUS_AGING_MEASUREMENT_MASK;

		/* Read CPR measurement results */
		result = cpr3_read(ctrl, CPR3_REG_RESULT1(0));
		quot_min = (result & CPR3_RESULT1_QUOT_MIN_MASK) >> CPR3_RESULT1_QUOT_MIN_SHIFT;
		quot_max = (result & CPR3_RESULT1_QUOT_MAX_MASK) >> CPR3_RESULT1_QUOT_MAX_SHIFT;
		sel_min = (result & CPR3_RESULT1_RO_MIN_MASK) >> CPR3_RESULT1_RO_MIN_SHIFT;
		sel_max = (result & CPR3_RESULT1_RO_MAX_MASK) >> CPR3_RESULT1_RO_MAX_SHIFT;
		...
		if (is_aging_measurement)
			quot_delta_results[aging_measurement_count++] = quot_delta_scaled;
	...
	filtered_count = aging_measurement_count - CPR3_AGING_MEASUREMENT_FILTER * 2;
	if (filtered_count > 0)
	{
		...
		for (i = 0; i < filtered_count; i++)
			quot_delta_scaled_sum += quot_delta_results[i + CPR3_AGING_MEASUREMENT_FILTER];

		aging_sensor->measured_quot_diff = quot_delta_scaled_sum / filtered_count;
...
cleanup:
	...
	cpr3_masked_write(ctrl, CPR3_REG_CPR_CTL,
										CPR3_CPR_CTL_COUNT_MODE_MASK | CPR3_CPR_CTL_COUNT_REPEAT_MASK,
										(ctrl->count_mode << CPR3_CPR_CTL_COUNT_MODE_SHIFT) | (ctrl->count_repeat << CPR3_CPR_CTL_COUNT_REPEAT_SHIFT));
																									...
																									int old_adjust_volt, int new_adjust_volt)
{
	...
	for (i = 0; i < vreg->corner_count; i++)
	{
		temp = (unsigned long long)old_adjust_volt * (unsigned long long)vreg->corner[i].aging_derate;
		do_div(temp, 1000);
		old_volt = temp;

		temp = (unsigned long long)new_adjust_volt * (unsigned long long)vreg->corner[i].aging_derate;
		do_div(temp, 1000);
		new_volt = temp;
		...
		for (j = 0; j < CPR3_RO_COUNT; j++)
		{
			...
			{
				vreg->corner[i].target_quot[j] += cpr3_quot_adjustment(
		...
		vreg->corner[i].ceiling_volt = vreg->corner[i].unaged_ceiling_volt + rounded_volt;
		vreg->corner[i].ceiling_volt = min(vreg->corner[i].ceiling_volt,
																			 vreg->corner[i].abs_ceiling_volt);
		vreg->corner[i].floor_volt = vreg->corner[i].unaged_floor_volt + rounded_volt;
		vreg->corner[i].floor_volt = min(vreg->corner[i].floor_volt,
																		 vreg->corner[i].ceiling_volt);
		vreg->corner[i].open_loop_volt = vreg->corner[i].unaged_open_loop_volt + rounded_volt;
		vreg->corner[i].open_loop_volt = min(vreg->corner[i].open_loop_volt,
																				 vreg->corner[i].ceiling_volt);
...
static int cpr3_regulator_aging_adjust(struct cpr3_controller *ctrl)
{
	...
	if (!ctrl->aging_required || !ctrl->cpr_enabled || ctrl->aggr_corner.ceiling_volt == 0 || ctrl->aggr_corner.ceiling_volt > ctrl->aging_ref_volt)
		return 0;
	...
	for (i = 0, vreg_count = 0; i < ctrl->thread_count; i++)
	{
		for (j = 0; j < ctrl->thread[i].vreg_count; j++)
		{
			...
			if (vreg->aging_allowed && vreg->vreg_enabled && vreg->current_corner > vreg->aging_corner)
				return 0;
		}
	}
	...
	for (i = 0, vreg_count = 0; i < ctrl->thread_count; i++)
	{
		for (j = 0; j < ctrl->thread[i].vreg_count; j++, vreg_count++)
		{
			...
			restore_current_corner[vreg_count] = vreg->current_corner;
			restore_vreg_enabled[vreg_count] = vreg->vreg_enabled;
		...
		if (!rc)
		{
			aging_volt =
					cpr3_voltage_adjustment(
							...
							ctrl->aging_sensor[i].measured_quot_diff - ctrl->aging_sensor[i].init_quot_diff);
...
cleanup:
	...
	for (i = 0, vreg_count = 0; i < ctrl->thread_count; i++)
	{
		...
		{
			vreg = &ctrl->thread[i].vreg[j];
			vreg->current_corner = restore_current_corner[vreg_count];
...
int cpr_regulator_get_corner_voltage(struct regulator_dev *rdev,
																		 int corner)
{
...
int cpr_regulator_set_corner_voltage(struct regulator_dev *rdev,
																		 int corner, int volt)
{
	struct cpr3_regulator *vreg = rdev_get_drvdata(rdev);
	struct cpr3_controller *ctrl = vreg->thread->ctrl;
	...
	if (corner >= 0 && corner <= vreg->corner_count)
	{
		...
		vreg->corner[corner].ceiling_volt = volt;
		vreg->corner[corner].floor_volt = volt - 200000;
...
static int cprh_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	return (reg & CPRH_STATUS_CORNER) + CPR3_CORNER_OFFSET;
}
...
static void cpr3_print_result(struct cpr3_thread *thread)
{
	...
	error_steps = (result[0] & CPR3_RESULT0_ERROR_STEPS_MASK) >> CPR3_RESULT0_ERROR_STEPS_SHIFT;
	error = (result[0] & CPR3_RESULT0_ERROR_MASK) >> CPR3_RESULT0_ERROR_SHIFT;
	negative = !!(result[0] & CPR3_RESULT0_NEGATIVE_MASK);

	quot_min = (result[1] & CPR3_RESULT1_QUOT_MIN_MASK) >> CPR3_RESULT1_QUOT_MIN_SHIFT;
	quot_max = (result[1] & CPR3_RESULT1_QUOT_MAX_MASK) >> CPR3_RESULT1_QUOT_MAX_SHIFT;
	ro_min = (result[1] & CPR3_RESULT1_RO_MIN_MASK) >> CPR3_RESULT1_RO_MIN_SHIFT;
	ro_max = (result[1] & CPR3_RESULT1_RO_MAX_MASK) >> CPR3_RESULT1_RO_MAX_SHIFT;

	step_quot_min = (result[2] & CPR3_RESULT2_STEP_QUOT_MIN_MASK) >> CPR3_RESULT2_STEP_QUOT_MIN_SHIFT;
	step_quot_max = (result[2] & CPR3_RESULT2_STEP_QUOT_MAX_MASK) >> CPR3_RESULT2_STEP_QUOT_MAX_SHIFT;
	sensor_min = (result[2] & CPR3_RESULT2_SENSOR_MIN_MASK) >> CPR3_RESULT2_SENSOR_MIN_SHIFT;
	sensor_max = (result[2] & CPR3_RESULT2_SENSOR_MAX_MASK) >> CPR3_RESULT2_SENSOR_MAX_SHIFT;
...
static irqreturn_t cpr3_irq_handler(int irq, void *data)
{
	...
	if (!up && !down)
	{
		...
		for (i = 0; i < CPR3_REGISTER_WRITE_DELAY_US / 10; i++)
		{
			...
			if (cpr_status & CPR3_CPR_STATUS_BUSY_MASK && (ctl & CPR3_CPR_CTL_LOOP_EN_MASK) == CPR3_CPR_CTL_LOOP_DISABLE)
				break;
			udelay(10);
		}
	...
	if (ctrl->proc_clock_throttle && last_volt == aggr->ceiling_volt && new_volt < last_volt)
		cpr3_write(ctrl, CPR3_REG_PD_THROTTLE,
							 ctrl->proc_clock_throttle);
	...
	corner = &ctrl->thread[0].vreg[0].corner[ctrl->thread[0].vreg[0].current_corner];
...
static int cpr3_debug_closed_loop_enable_set(void *data, u64 val)
{
	...
	{
		cpr3_debug(ctrl, "closed-loop=%s\n", enable && ctrl->use_hw_closed_loop ? "enabled" : "disabled");
	}
done:
	mutex_unlock(&ctrl->lock);
	return 0;
...
static int cpr3_debug_hw_closed_loop_enable_set(void *data, u64 val)
{
	...
	else if (!ctrl->use_hw_closed_loop && ctrl->ctrl_type == CPR_CTRL_TYPE_CPR3)
	{
	...
	if (ctrl->ctrl_type != CPR_CTRL_TYPE_CPRH)
	{
		...
		for (i = 0; i < ctrl->thread_count; i++)
		{
			for (j = 0; j < ctrl->thread[i].vreg_count; j++)
			{
				...
				for (k = 0; k < vreg->corner_count; k++)
					vreg->corner[k].last_volt = vreg->corner[k].open_loop_volt;
			}
		}
		...
		cpr3_debug(ctrl, "CPR mode=%s\n",
							 use_hw_closed_loop ? "HW closed-loop" : "SW closed-loop");
	}
	else
	{
		cpr3_debug(ctrl, "CPR mode=%s\n",
							 ctrl->cpr_allowed_sw && use_hw_closed_loop ? "full HW closed-loop" : "open-loop");
	}
done:
	mutex_unlock(&ctrl->lock);
	return 0;
...
static void cpr3_regulator_debugfs_ctrl_add(struct cpr3_controller *ctrl)
{
	...
	if (ctrl->aging_required || ctrl->aging_succeeded || ctrl->aging_failed)
	{
...
static int cpr3_regulator_init_ctrl_data(struct cpr3_controller *ctrl)
{
	...
	ctrl->aggr_corner.last_volt = regulator_get_voltage(ctrl->vdd_regulator);
...
static int cpr3_regulator_init_vreg_data(struct cpr3_regulator *vreg)
{
	...
	for (i = 0; i < vreg->corner_count; i++)
	{
		...
		if (init_aging)
		{
			vreg->corner[i].unaged_floor_volt = vreg->corner[i].floor_volt;
			vreg->corner[i].unaged_ceiling_volt = vreg->corner[i].ceiling_volt;
			vreg->corner[i].unaged_open_loop_volt = vreg->corner[i].open_loop_volt;
		}
	...
	if (vreg->aging_allowed && vreg->corner[vreg->aging_corner].ceiling_volt > vreg->thread->ctrl->aging_ref_volt)
	{
																							 ...
																							 unsigned long action, void *hcpu)
{
	...
	if (action == CPU_ONLINE && cpumask_test_cpu(cpu, &ctrl->irq_affinity_mask))
...
static int cpr3_regulator_validate_controller(struct cpr3_controller *ctrl)
{
	...
	else if (ctrl->sensor_count <= 0 || ctrl->sensor_count > CPR3_MAX_SENSOR_COUNT)
	{
	...
	if (ctrl->aging_required)
	{
		for (i = 0; i < ctrl->aging_sensor_count; i++)
		{
			if (ctrl->aging_sensor[i].sensor_id >= ctrl->sensor_count)
			{
														...
														struct cpr3_controller *ctrl)
{
	...
	if (ctrl->irq && !cpumask_empty(&ctrl->irq_affinity_mask))
	{
		...
		ctrl->cpu_hotplug_notifier.notifier_call = cpr3_regulator_cpu_hotplug_callback;
